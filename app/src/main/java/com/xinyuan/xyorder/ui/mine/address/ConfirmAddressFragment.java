...
public class ConfirmAddressFragment extends BaseFragment implements AMap.OnPOIClickListener,
...
//    ImageView iv_current_postion;
    ...
    public static ConfirmAddressFragment newInstance(AddressInfo info) {
    ...
    @Override
    public void onEnterAnimationEnd(final Bundle savedInstanceState) {
...
//        uiSettings.setLogoBottomMargin(-50);//隐藏logo
        ...
        if (XEmptyUtils.isEmpty(info)) {
            if (DataUtil.checkLocation(getActivity())) {
                lat = DataUtil.getLocation(getActivity()).getLatitude();
                lng = DataUtil.getLocation(getActivity()).getLongitude();
            }
...
//            tv_address.setText(regeocodeAddress.getFormatAddress());
            tv_address.setText(regeocodeAddress.getPois().get(0).getTitle());
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.tv_confirm_address:
                ...
                AddressInfo info = new AddressInfo(address, lat, lng, tv_detail_address.getText().toString(), tv_address.getText().toString());
                EventBus.getDefault().post(new AddressPageEvent(AddressPageEvent.CHOOSELOCATION, info));
